...
from mfax.algos.hsm.utils.make_act import MFRecurrentActorWrapper  # noqa: E402
...
class args:
    ...
    state_type: str = "states"  # "indices"
    discount_factor: float = 0.95  # 0.99
    ...
    def _train_step(runner_state, unused):
        ...
        def _mf_rollout(actor_params, rng):
            ...
            def _mf_transition(runner_state, unused):
                aggregate_s, aggregate_obs, aggregate_hidden, aggregate_done, rng = (
                    runner_state
                )
                rng, rng_env, rng_next = jax.random.split(rng, 3)

                # --- select action ---
                prob_a, next_hidden = mf_policy_net.dist_prob(
                    actor_params,
